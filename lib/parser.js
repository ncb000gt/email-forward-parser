...
class Parser {
  ...
  parseBody(body, forwarded = false) {
    ...
    if (forwarded === true) {
      ...
      //   Outlook Live / 365)
      _match = loopRegexes(this.__regexes.original_from, _body, Symbol.split);
    ...
    for (var _key in REGEXES) {
      ...
      if (Array.isArray(_entry)) {
        ...
        for (var _i = 0; _i < _entry.length; _i++) {
          ...
          this.__regexes[_key].push(new RegExp(_regex));
        }
      } else {
        var _regex = _entry;
  ...
  __parseOriginalBody(text) {
    ...
    for (var _i = 0; _i < _regexes.length; _i++) {
      _match = loopRegexes(_regexes[_i], text, Symbol.split);
  ...
  __parseOriginalFrom(text, body) {
    ...
    if (_match && _match.length === 4 && _match.groups) {
      ...
      return this.__prepareMailbox(_address, _name);
    }
      ...
      if (_mailboxesLine) {
        ...
        while (_mailboxesLine) {
          ...
          if (_mailboxMatch && _mailboxMatch.length > 0) {
            ...
            _mailboxes.push(this.__prepareMailbox(_address, _name));
            ...
            _mailboxes.push(this.__prepareMailbox(_mailboxesLine, null));
